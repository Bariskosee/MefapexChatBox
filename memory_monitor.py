            ...
            if tracemalloc.is_tracing():
                ...
                for stat in top_stats:
                    top_allocations.append((
                        stat.traceback.format()[-1] if stat.traceback.format() else "unknown",
...
class MemoryLeakDetector:
    """Utilities for detecting specific types of memory leaks"""
    ...
    def check_circular_references():
        ...
        if circular_refs:
            logger.warning(f"Found {len(circular_refs)} potential circular references")
            
        return circular_refs
        
    @staticmethod
    def analyze_large_objects(threshold_mb: float = 10.0):
        """Analyze large objects in memory"""
        logger.info(f"🔍 Analyzing objects larger than {threshold_mb}MB...")
        ...
        if large_objects:
            logger.warning(f"Found {len(large_objects)} large objects:")
            for obj in large_objects[:5]:  # Show top 5
                logger.warning(f"  {obj['type']}: {obj['size_mb']:.2f}MB")
